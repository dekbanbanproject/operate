...
class ManagermpayController extends Controller
{
    ...
    public function mpay_service_receive_save(Request $request)
    {
        ...
        #ลูปบันทึก และอัพเดตจำนวนใน set,โควต้า
        #เพิ่ม set not sterlize
        #ลบ โควต้า ยกเว้นสถานะรับเข้าใหม่
    ...
    public function mpay_service_stickerprint_save(Request $request)
    {
        ...
        $data['hostpital_name']          = (!empty($info_org))?$info_org->ORG_NAME:''; 
   ...
   public function test_print()
   {
    ...
    {
        $count = cpay_setequpment_sub::where('STORE_ID', $request->STORE_ID)->count();
        if ($count > 0) {
            Session::flash('err', 'ไม่สามารถบันทึกข้อมูลสำเร็จ เนื่องจากมีการใช้งานทะเบียนวัสดุนี้แล้ว');
        } else {
            $person                                             = Person::find(Auth::user()->PERSON_ID);
            $cpay_setequpment_subsave                           = new cpay_setequpment_sub();
            $cpay_setequpment_subsave->STORE_ID                 = $request->STORE_ID;
            $cpay_setequpment_subsave->STORE_CODE               = $request->STORE_CODE;
    ...
    public function mpay_setting_subset_medequipment_update(Request $request)
    {
        ...
        $cpay_setequpment_subupdate->STORE_ID                 = $request->STORE_ID;
        $cpay_setequpment_subupdate->STORE_CODE               = $request->STORE_CODE;
    ...
    public function mpay_setting_medequipment_update(Request $request)
    {
        ...
        $setequp_update->CPAY_SET_NOT_STERILE_QUANTITY = 0;
        $setequp_update->CPAY_SET_STERILE_QUANTITY     = 0;
