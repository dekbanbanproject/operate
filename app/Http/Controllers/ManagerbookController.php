...
class ManagerbookController extends Controller
{
    public function dashboard()
    {
        ...
        $data['amount_receive'] = Bookindex::where('BOOK_YEAR_ID','=',$year)->where('BOOK_USE','=','true')->count();
        $data['amount_sent'] = Bookindexinside::where('BOOK_YEAR_ID','=',$year)->where('BOOK_USE','=','true')->count();
        $data['amount_command'] = Bookindexcommand::where('BOOK_YEAR_ID','=',$year)->where('BOOK_USE','=','true')->count();
        $data['amount_announce'] = Bookindexannounce::where('BOOK_YEAR_ID','=',$year)->where('BOOK_USE','=','true')->count();
    ...
    public function bookreceipt_excel()
    {
        $iduser  = Auth::user()->PERSON_ID; 
        $date = date('Y-m-d');
        ...
        ->leftJoin('hrd_prefix','hrd_person.HR_PREFIX_ID','=','hrd_prefix.HR_PREFIX_ID')
        ->leftJoin('gbook_index_send_leader','gbook_index.BOOK_ID','=','gbook_index_send_leader.BOOK_LD_ID')
        ->WhereBetween('DATE_SAVE',[$displaydate_bigen,$displaydate_end]) 
        ->where('BOOK_USE','=','true')
        ->orderBy('gbook_index.BOOK_ID', 'desc') 
        ->get();
...
if($request->BOOK_NUMBER <> '' && $request->BOOK_NAME <> ''){
        ...
        $addreceipt->BOOK_NUM_IN = $request->BOOK_NUM_IN;
...
if($request->BOOK_NUMBER <> '' && $request->BOOK_NAME <> ''){
    ...
    public function bookinside_excel()
    {
       ...
       $displaydate_end = ($yearbudget-543).'-09-30';
        ...
        $infobookinside = Bookindexinside::WhereBetween('DATE_SAVE',[$displaydate_bigen,$displaydate_end]) 
        ->where('BOOK_USE','=','true')
        ->orderBy('BOOK_ID', 'desc') 
        ->get();
...
if($request->BOOK_NUMBER <> '' && $request->BOOK_NAME <> ''){
          ...
          $addinside->BOOK_NUM_IN = $request->BOOK_NUM_IN;
...
if($request->BOOK_NUMBER <> '' && $request->BOOK_NAME <> ''){
     ...
     public function saverpresentinside(Request $request)
     {
       ...
       public function bookcommand_excel()
       {
             $date = date('Y-m-d');
           ...
           $infobookcommand = Bookindexcommand::WhereBetween('DATE_SAVE',[$displaydate_bigen,$displaydate_end]) 
           ->orderBy('BOOK_ID', 'desc') 
           ->get();
   ...
   if($request->ORG_ID != '' || $request->ORG_ID != null){
    ...
    public function saverpresentcomdoc(Request $request)
    {
        ...
        public function bookannounce_excel()
        {
            $date = date('Y-m-d');
            ...
            $infobookannounce = Bookindexannounce::WhereBetween('DATE_SAVE',[$displaydate_bigen,$displaydate_end]) 
            ->orderBy('BOOK_ID', 'desc') 
            ->get();
