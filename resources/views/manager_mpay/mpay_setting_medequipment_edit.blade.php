...
<div class="block block-rounded block-bordered mb-4" style="width: 95%;margin:auto">
    <div class="block-header block-header-default">
        <h3 class="block-title">แก้ไขชุดอุปกรณ์</h3>
    </div>
    <div class="block-content">
                <form action="{{route('mpay.mpay_setting_medequipment_update')}}" method="POST">
                @csrf
                <input type="hidden" name="CPAY_SET_ID" required value="{{$setequpment->CPAY_SET_ID}}">
                    <!-- Basic Elements -->
                    <div class="row">
                        <div class="col-lg-4">
                            <div class="form-group">
                                ...
                                <select class="js-select2 form-control" id="store_select" name="store_select"
                                    ...
                                    <?php
                                            ...
                                            ?>
                                    <option value="{{$row->STORE_ID}}" data-record="{{$data_record}}">
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="form-group mb-1">
                                ...
                                <input type="text" class="js-maxlength form-control js-maxlength-enabled f-kanit" id="CPAY_SET_NAME_INSIDE" name="CPAY_SET_NAME_INSIDE" placeholder="ชื่อภายใน" required value="{{$setequpment->CPAY_SET_NAME_INSIDE}}">
                            </div>
                            <div class="form-group mb-1">
                                <label for="CPAY_SET_NAME_TH">ชื่อไทย</label> <span class="text-danger">*</span>
                                <input type="text" class="js-maxlength form-control js-maxlength-enabled f-kanit" id="CPAY_SET_NAME_TH" name="CPAY_SET_NAME_TH" placeholder="ชื่อไทย" required value="{{$setequpment->CPAY_SET_NAME_TH}}">
                            </div>
                            <div class="form-group mb-1">
                                <label for="CPAY_SET_NAME_EN">ชื่ออังกฤษ</label>
                                <input type="text" class="js-maxlength form-control js-maxlength-enabled f-kanit" id="CPAY_SET_NAME_EN" name="CPAY_SET_NAME_EN" placeholder="ชื่ออังกฤษ" value="{{$setequpment->CPAY_SET_NAME_EN}}">
                            </div>
                            <div class="form-group mb-1">
                                <label for="CPAY_SET_BRAND">ชื่อยี่ห้อ</label>
                                <input type="text" class="js-maxlength form-control js-maxlength-enabled f-kanit" id="CPAY_SET_BRAND" name="CPAY_SET_BRAND" placeholder="ชื่อยี่ห้อ" value="{{$setequpment->CPAY_SET_BRAND}}">
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="form-group mb-1">
                                <label for="CPAY_UNIT_ID">หน่วยนับ</label> <span class="text-danger">*</span>
                                <select class="js-select2 form-control f-kanit" id="CPAY_UNIT_ID" name="CPAY_UNIT_ID" data-placeholder="หน่วยนับ" required value="{{$setequpment->CPAY_UNIT_ID}}">
                                    <option></option>
                                    @foreach($unit as $row)
                                        @if($row->CPAY_UNIT_ID === $setequpment->CPAY_UNIT_ID)
                                            <option value="{{$row->CPAY_UNIT_ID}}" selected>{{$row->CPAY_UNIT_NAME}}</option>
                                        @else
                            ...
                            <div class="form-group mb-1">
                                ...
                                <input type="number" class="js-maxlength form-control js-maxlength-enabled f-kanit" id="CPAY_SET_PRICE" name="CPAY_SET_PRICE" min="0" value="0" required value="{{$setequpment->CPAY_SET_PRICE}}">
                            </div>
                            <div class="form-group mb-1">
                                <label for="CPAY_SET_STERILE_DAY">ระยะเวลาปลอดเชื้อ (วัน)</label> <span class="text-danger">*</span>
                                <input type="number" class="js-maxlength form-control js-maxlength-enabled f-kanit" id="CPAY_SET_STERILE_DAY" name="CPAY_SET_STERILE_DAY" min="1" value="1" required value="{{$setequpment->CPAY_SET_STERILE_DAY}}">
                            </div>
                            <div class="form-group mb-1">
                                ...
                                <select class="form-control f-kanit" id="CPAY_TYPEMACH_ID" name="CPAY_TYPEMACH_ID" required>
                                    ...
                                    @foreach($typemach as $row)
                                        @if($row->CPAY_TYPEMACH_ID === $setequpment->CPAY_TYPEMACH_ID)
                            ...
                            <div class="form-group mb-1">
                                ...
                                <select class="form-control f-kanit" id="CPAY_GSET_ID" name="CPAY_GSET_ID" required>
                                    ...
                                    @if($row->CPAY_GSET_ID === $setequpment->CPAY_GSET_ID)
                                    <option value="{{$row->CPAY_GSET_ID}}" selected>{{$row->CPAY_GSET_NAME}}</option>
                                    @else
                        ...
                        <div class="col-lg-12">
                            <div class="form-group mb-1">
                                ...
                                <textarea type="text" style="height:90px;"
                                    ...
                                    name="CPAY_SET_DETAIL" maxlength="255" placeholder="รายละเอียด...">{{$setequpment->CPAY_SET_DETAIL}}</textarea>
                            </div>
                        </div>
                        <div class="col-lg-6 mb-1">
                                <div class="form-check form-check-inline mb-1">
                                                <input class="form-check-input" type="checkbox" value="1" {{($setequpment->CPAY_SET_HAVE_LIST)?'checked':''}} id="CPAY_SET_HAVE_LIST" name="CPAY_SET_HAVE_LIST">
                    ...
                    <div class="row items">
                        <div class="col-lg-12">
                            <table class="table table-striped table-bordered {{($setequpment->CPAY_SET_HAVE_LIST)?'':'d-none'}}">
                                <thead>
                                    <tr>
                                ...
                                <tbody class="tbody">
                                    <!-- add row form js -->
                                    @foreach($setequpment_list as $rowset)
                                    <tr>
                                    <td width="50%" class="text-center">
                                        ...
                                        @if($row->CPAY_SET_SUB_ID === $rowset->CPAY_SET_SUB_ID)
                                        <option value="{{$row->CPAY_SET_SUB_ID}}" selected>{{$row->CPAY_SET_SUB_NAME_INSIDE}}</option>
                                        @else
                                    ...
                                    <td class="text-center" width="100px"><input class="form-control text-center" name="CPAY_SETLIST_QUANTITY[]" type="number" min="1" value="{{$rowset->CPAY_SETLIST_QUANTITY}}"></td>
