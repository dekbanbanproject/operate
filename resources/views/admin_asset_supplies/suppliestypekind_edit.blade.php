...
?>
                ...
                <h2 class="content-heading pt-0" style="font-family: 'Kanit', sans-serif;">แก้ไขข้อมูลประเภทพัสดุ</h2>    

    
        <form  method="post" action="{{ route('admin.updatesuppliestypekind') }}" enctype="multipart/form-data">
        @csrf
        <div class="row push">

        <input type="hidden" name = "SUP_TYPE_KIND_ID"  id="SUP_TYPE_KIND_ID" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;" value="{{$infotypekind->SUP_TYPE_KIND_ID}}">
     ...
     <div class="col-lg-2">
      ...
      <select name="SUP_TYPE_MASTER_ID" id="SUP_TYPE_MASTER_ID" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;font-size: 13px;" onchange="check_sub_type_master_id();">
            <option value="">--กรุณาเลือกชนิดพัสดุ--</option>
             ...
             @if($infotypekind->SUP_TYPE_MASTER_ID == $typemaster ->SUP_TYPE_MASTER_ID )
      ...
      <div class="col-lg-5">
      <input  name = "SUP_TYPE_KIND_NAME"  id="SUP_TYPE_KIND_NAME" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;" value="{{$infotypekind->SUP_TYPE_KIND_NAME}}" onkeyup="check_sub_type_kind_name();">
      ...
      <div class="col-lg-10">
      <input  name = "SUP_TYPE_KIND_DETAIL"  id="SUP_TYPE_KIND_DETAIL" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;" value="{{$infotypekind->SUP_TYPE_KIND_DETAIL}}" onkeyup="check_sub_type_kind_detail();">
      <div style="color: red; font-size: 16px;" id="sub_type_kind_detail"></div> 
        ...
        <button type="submit"  class="btn btn-hero-sm btn-hero-info" >บันทึกแก้ไขข้อมูล</button>
         <a href="{{ url('admin_asset_supplies/setupsuppliestypekind')  }}" class="btn btn-hero-sm btn-hero-danger" onclick="return confirm('ต้องการที่จะยกเลิกการแก้ไขข้อมูล ?')" >ยกเลิก</a> 
         </div>    
