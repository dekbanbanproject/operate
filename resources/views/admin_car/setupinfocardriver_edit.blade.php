...
?>
                ...
                <h2 class="content-heading pt-0" style="font-family: 'Kanit', sans-serif;">แก้ไขข้อมูลพนักงานขับรถ</h2>    

    
        <form  method="post" action="{{ route('admin.updatecardriver') }}" enctype="multipart/form-data">
        @csrf
        <div class="row push">
    ...
    <div class="col-lg-2">
      ...
      <div class="col-lg-3">
      <select name="PERSON_ID" id="PERSON_ID" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;">
      <option value="">--กรุณาเลือกพนักงาน--</option>
            @foreach ($PERSONALLs as $PERSONALL) 
                  @if($PERSONALL ->ID == $infocardriver->PERSON_ID)
      ...
      <input  type="hidden" name = "DRIVER_ID"  id="DRIVER_ID" class="form-control input-lg" value="{{$infocardriver->DRIVER_ID}}">
      </div>
        ...
        <button type="submit"  class="btn btn-hero-sm btn-hero-info" >บันทึกข้อมูล</button>
         <a href="{{ url('admin_car/setupcardriver')  }}" class="btn btn-hero-sm btn-hero-danger" onclick="return confirm('ต้องการที่จะยกเลิกการเพิ่มข้อมูล ?')" >ยกเลิก</a> 
         </div>    
