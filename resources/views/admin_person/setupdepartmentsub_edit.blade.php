...
?>
                ...
                <h2 class="content-heading pt-0" style="font-family: 'Kanit', sans-serif;">แก้ไขข้อฝ่าย/แผนก</h2>    

    
        <form  method="post" action="{{ route('admin.updatepersondepartmentsub') }}" enctype="multipart/form-data">
        @csrf
        <div class="row push">
       
      <div>
      <label >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;รหัส {{ $infodepartmentsub->HR_DEPARTMENT_SUB_ID }}</label>
      <input type = "hidden"  name = "HR_DEPARTMENT_SUB_ID"  id="HR_DEPARTMENT_SUB_ID" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;" value="{{ $infodepartmentsub->HR_DEPARTMENT_SUB_ID }}">
      </div>
      <div class="col-lg-1">
      <label >ฝ่าย/แผนก</label>
      </div>
      <div class="col-lg-3">
      <input  name = "HR_DEPARTMENT_SUB_NAME"  id="HR_DEPARTMENT_SUB_NAME" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;"  value="{{ $infodepartmentsub->HR_DEPARTMENT_SUB_NAME }}" onkeyup="check_hr_department_subname();" required>
      ...
      <option value="">--กรุณาเลือกกลุ่มงาน--</option>
            @foreach ($infodepartments as $infodepartment)
                @if( $infodepartment ->HR_DEPARTMENT_ID == $infodepartmentsub->HR_DEPARTMENT_ID)
    ...
    <input  type="hidden" name = "ACTIVE"  id="ACTIVE" class="form-control input-lg" value="True">
        ...
        <button type="submit"  class="btn btn-hero-sm btn-hero-info" >บันทึกแก้ไขข้อมูล</button>
         <a href="{{ url('admin_person/setupinfodepartment')  }}" class="btn btn-hero-sm btn-hero-danger" onclick="return confirm('ต้องการที่จะยกเลิกการแก้ไขข้อมูล ?')" >ยกเลิก</a> 
         </div>    
