...
?>
        ...
        <form  method="post" action="{{ route('admin.savecommittee') }}" enctype="multipart/form-data">
        @csrf
        <div class="row push">
    ...
    <div class="col-lg-1">   
      ...
      <input  name = "TEAM_POSITION_ETC"  id="TEAM_POSITION_ETC" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;" >
      </div>
      ...
      <input  type="hidden" name = "TEAM_ID"  id="TEAM_ID" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;" value="{{$teamid->HR_TEAM_ID}}" >
        ...
        <button type="submit"  class="btn btn-hero-sm btn-hero-info" ><i class="fas fa-save"></i>   บันทึกข้อมูล</button>
         <a href="{{ url('admin_person_H/setupinfopersonteam/committee/'.$teamid->HR_TEAM_ID)  }}" class="btn btn-hero-sm btn-hero-danger" onclick="return confirm('ต้องการที่จะยกเลิกการเพิ่มข้อมูล ?')" ><i class="fas fa-window-close"></i>  ยกเลิก</a> 
         </div>    
