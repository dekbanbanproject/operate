...
?>
                ...
                <h2 class="content-heading pt-0" style="font-family: 'Kanit', sans-serif;">แก้ไขข้อหน่วยงาน</h2>    

    
        <form  method="post" action="{{ route('admin.updatepersondepartmentsubsub') }}" enctype="multipart/form-data">
        @csrf
        <div class="row push">
      
       <div>
      <label >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;รหัส {{ $infodepartmentsubsub->HR_DEPARTMENT_SUB_SUB_ID }}</label>
      <input type = "hidden"  name = "HR_DEPARTMENT_SUB_SUB_ID"  id="HR_DEPARTMENT_SUB_SUB_ID" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;" value="{{ $infodepartmentsubsub->HR_DEPARTMENT_SUB_SUB_ID }}">
      </div>
      ...
      <div class="col-lg-3">
      <input  name = "HR_DEPARTMENT_SUB_SUB_NAME"  id="HR_DEPARTMENT_SUB_SUB_NAME" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;font-size: 13px;" value="{{ $infodepartmentsubsub->HR_DEPARTMENT_SUB_SUB_NAME }}" onkeyup="check_hr_department_subsubname();" required>
      ...
      <option value="">--กรุณาเลือกฝ่าย/แผนก--</option>
            @foreach ($infodepartmentsubs as $infodepartmentsub) 
                  @if($infodepartmentsubsub->HR_DEPARTMENT_SUB_ID == $infodepartmentsub ->HR_DEPARTMENT_SUB_ID)
                  <option value="{{ $infodepartmentsub ->HR_DEPARTMENT_SUB_ID  }}" selected>{{ $infodepartmentsub-> HR_DEPARTMENT_SUB_NAME }}</option>
                  @else
    ...
    <div>
      ...
      <option value="">--กรุณาเลือกหัวหน้ากลุ่ม--</option>
            @foreach ($infopersons as $infoperson) 
                  @if($infoperson ->ID == $infodepartmentsubsub->LEADER_HR_ID )
    ...
    <input  type="hidden" name = "ACTIVE"  id="ACTIVE" class="form-control input-lg" value="True">
      ...
      <div class="col-lg-1">
                  ...
                  <div class="col-lg-3">
                  <input  name = "DEP_CODE"  id="DEP_CODE" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;font-size: 13px;" value="{{ $infodepartmentsubsub->DEP_CODE }}" required>
...
<div class="col-lg-1">
  ...
  <div class="col-lg-7">
  <input  name = "LINE_TOKEN"  id="LINE_TOKEN" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;font-size: 13px;" value="{{ $infodepartmentsubsub->LINE_TOKEN }}">
        ...
        <button type="submit"  class="btn btn-hero-sm btn-hero-info" >บันทึกแก้ไขข้อมูล</button>
         <a href="{{ url('admin_person/setupinfoinstitution')  }}" class="btn btn-hero-sm btn-hero-danger" onclick="return confirm('ต้องการที่จะยกเลิกการแก้ไขข้อมูล ?')" >ยกเลิก</a> 
         </div>    
