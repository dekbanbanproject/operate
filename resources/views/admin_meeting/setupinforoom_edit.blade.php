...
?>
                ...
                <h2 class="content-heading pt-0" style="font-family: 'Kanit', sans-serif;">แก้ไขข้อมูลห้องประชุม</h2>    

    
        <form  method="post" action="{{ route('admin.updateroom') }}"inforoom enctype="multipart/form-data">
        @csrf
        <div class="row push">
    ...
    <div class="col-lg-4">
      ...
      <input  name = "ROOM_NAME"  id="ROOM_NAME" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;" value="{{$inforoom->ROOM_NAME}}" onkeyup="check_room_name();">
      <div style="color: red; font-size: 16px;" id="room_name"></div>
    </div>
      <div class="form-group">
      <label >จองล่วงหน้า (วัน)</label>
      <input  name = "GET_BEFORE"  id="GET_BEFORE" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;" value="{{$inforoom->GET_BEFORE}}" OnKeyPress="return chkNumber(this)" onkeyup="check_get_before();">
      <div style="color: red; font-size: 16px;" id="get_before"></div> 
    </div>
      <div class="form-group">
        <img src="data:image/png;base64,{{ chunk_split(base64_encode($inforoom->IMG1)) }}" id="image_upload_preview1"   height="250px" width="100%"/>
        </div>                             
        <div class="form-group">
    ...
    <div class="col-lg-4">
      ...
      <input  name = "ROOM_LOCATION"  id="ROOM_LOCATION"  class="form-control input-lg "  style=" font-family: 'Kanit', sans-serif;" value="{{$inforoom->ROOM_LOCATION}}" onkeyup="check_room_location();">
      <div style="color: red; font-size: 16px;" id="room_location"></div> 
    </div>
      <div class="form-group">
      <label >ความจุ (คน)</label>
      <input  name = "CONTAIN"  id="CONTAIN"  class="form-control input-lg"  style=" font-family: 'Kanit', sans-serif;" value="{{$inforoom->CONTAIN}}" OnKeyPress="return chkNumber(this)" onkeyup="check_contain();">
      <div style="color: red; font-size: 16px;" id="contain"></div>
    </div>
      <div class="form-group">
        <img src="data:image/png;base64,{{ chunk_split(base64_encode($inforoom->IMG2)) }}" id="image_upload_preview2"   height="250px" width="100%"/>
        </div>                             
        <div class="form-group">
    ...
    <div class="col-lg-3">
      ...
      <select name="ROOM_PERSON_ID" id="ROOM_PERSON_ID" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;font-size: 13px;" onchange="check_room_person_id();">>
                    <option value="">--กรุณาเลือกผู้รับผิดชอบ--</option>
                        @foreach ($inforpersons as $inforperson)  
                        @if($inforperson->ID == $inforoom->ROOM_PERSON_ID)
      ...
      <input  name = "ROOM_DETAIL"  id="ROOM_DETAIL" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;" value="{{$inforoom->ROOM_DETAIL}}">
      </div>
                        <div class="form-group">
                            ...
                            <input name="ROOM_COLOR" type="color" id="ROOM_COLOR" class="form-control input-lg"
                                style=" font-family: 'Kanit', sans-serif;" value="{{$inforoom->ROOM_COLOR}}">
                        </div>
    </div>
       
    <input  type="hidden" name = "ROOM_ID"  id="ROOM_ID" class="form-control input-lg" value="{{$inforoom->ROOM_ID}}">

      </div></div>
        <div class="modal-footer">
        <div align="right">
        <button type="submit"  class="btn btn-hero-sm btn-hero-info" >บันทึกแก้ไขข้อมูล</button>
         <a href="{{ url('admin_meeting/setupinforoom')  }}" class="btn btn-hero-sm btn-hero-danger" onclick="return confirm('ต้องการที่จะยกเลิกการแก้ไขข้อมูล ?')" >ยกเลิก</a> 
         </div>    
